...
export function запуститьРасписание({ bot, supa, openai, ownerTgId, безКэша, ДИРЕКТОР_ПРОМТ, ожидаемОтвет, ждёмПлан, ждёмРазбор }) {
  if (!ownerTgId) {
  ...
  cron.schedule('0 21 * * *', async () => {
    ...
    // Любой свободный ответ на чек-ин → вечерний разбор (без нажатия кнопки)
    ждёмРазбор?.set(String(ownerTgId), true);
...
export function зарегистрироватьОбработчики({ bot, supa, openai, ownerTgId, ждёмРазбор }) {
  ...
  bot.callbackQuery('checkin:text', async (ctx) => {
    await ctx.answerCallbackQuery();
    ждёмРазбор?.set(String(ctx.from.id), true);
    await ctx.reply('📝 Напиши свободным текстом что сделал/не сделал сегодня — закрою/перенесу задачи и соберу план на завтра.');
  });

  bot.callbackQuery('checkin:voice', async (ctx) => {
    await ctx.answerCallbackQuery();
    ждёмРазбор?.set(String(ctx.from.id), true);
    await ctx.reply('🎙️ Расскажи голосом как прошёл день — разберу задачи и спланирую завтра.');
  });
