...
const ЖДЁМ_ПЛАН = new Map();      // userId → true (следующий текст/голос → AI-коуч сгенерит план дня)
const ЖДЁМ_РАЗБОР = new Map();    // userId → true (следующий текст/голос → вечерний разбор дня)
const coachDeps = (chatId) => ({ bot, supa, openai, ownerTgId: chatId, безКэша, ждёмПлан: ЖДЁМ_ПЛАН });
...
📑 report_request — триггеры и поле "kind":
  ...
  • «деньги», «финансы», «платежи», «что горит по деньгам», «долги», «лиды» → kind="money"
...
bot.command('money',     async (ctx) => { await ctx.replyWithChatAction('typing'); await отправитьДеньги(отчётDeps(ctx)); });

bot.callbackQuery(/^rep:(dashboard|kanban|plan|report|money)$/, async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.replyWithChatAction('typing');
...
// ── ЦЕЛИ + ПОДБИВКА (план/факт/цель) ──────────────────────────────────────────
bot.command('goal', async (ctx) => {
  const с = ctx.match?.trim();
  if (!с) return ctx.reply('🎯 Формат: `/goal Заголовок | target | unit | deadline`\nНапр: `/goal Выручка | 1000000 | ₽ | 2026-12-31`', { parse_mode:'Markdown' });
  const ц = await добавитьЦель(supa, с);
  await ctx.reply(ц ? `✅ Цель добавлена: *${ц.title}* → ${ц.target}${ц.unit?' '+ц.unit:''}${ц.deadline?` до ${ц.deadline}`:''}` : '⚠️ Не разобрал. Нужно минимум: Заголовок | target', { parse_mode:'Markdown' });
});
bot.command('goalp', async (ctx) => {
  const [n, ...rest] = (ctx.match?.trim()||'').split(/\s+/);
  const ц = await обновитьФакт(supa, Number(n), rest.join(' '));
  await ctx.reply(ц ? `✅ *${ц.title}*: факт ${ц.current}/${ц.target}${ц.unit?' '+ц.unit:''}` : '⚠️ Цель №'+n+' не найдена. /goals — список.', { parse_mode:'Markdown' });
});
bot.command('meeting', async (ctx) => {
  const т = ctx.match?.trim();
...
bot.command('coach', async (ctx) => {
  await отправитьПлан(coachDeps(ctx.chat.id), ctx.match?.trim() || '');
});
// Вечерний разбор вручную: /razbor <как прошёл день> (или без текста — жду ответ)
bot.command('razbor', async (ctx) => {
  const текст = ctx.match?.trim();
  if (текст) return вечернийРазбор(coachDeps(ctx.chat.id), текст);
  ЖДЁМ_РАЗБОР.set(String(ctx.from.id), true);
  await ctx.reply('🌙 Расскажи как прошёл день (текст/голос) — закрою/перенесу задачи и спланирую завтра.');
});
bot.callbackQuery('coach:gen', async (ctx) => {
  await ctx.answerCallbackQuery();
  ЖДЁМ_ПЛАН.delete(String(ctx.from.id));
  await отправитьПлан(coachDeps(ctx.chat.id), '');
});
bot.callbackQuery('coach:voice', async (ctx) => {
  await ctx.answerCallbackQuery();
  ЖДЁМ_ПЛАН.set(String(ctx.from.id), true);
  await ctx.reply('🎙 Жду голосовое — расскажи план на день, соберу распорядок.');
});
...
bot.on('message:voice', async (ctx) => {
  ...
  try {
    ...
    if (ЖДЁМ_ПЛАН.has(ctx.from.id) || ЖДЁМ_ПЛАН.has(String(ctx.from.id))) {
      ЖДЁМ_ПЛАН.delete(ctx.from.id); ЖДЁМ_ПЛАН.delete(String(ctx.from.id));
      await отправитьПлан(coachDeps(ctx.chat.id), рез.text);
      return;
    }
    if (ЖДЁМ_РАЗБОР.has(ctx.from.id) || ЖДЁМ_РАЗБОР.has(String(ctx.from.id))) {
      ЖДЁМ_РАЗБОР.delete(ctx.from.id); ЖДЁМ_РАЗБОР.delete(String(ctx.from.id));
      await вечернийРазбор(coachDeps(ctx.chat.id), рез.text);
      return;
    }
...
bot.on('message:audio', async (ctx) => {
  ...
  try {
    ...
    await разобратьСозвон(coachDeps(ctx.chat.id), рез.text);
  } catch (err) {
...
bot.on('message:text', async (ctx) => {
  ...
  // Ждём план дня (утренний коуч) → этот текст = вводные для распорядка
  if (ЖДЁМ_ПЛАН.has(ctx.from.id) || ЖДЁМ_ПЛАН.has(String(ctx.from.id))) {
    ЖДЁМ_ПЛАН.delete(ctx.from.id); ЖДЁМ_ПЛАН.delete(String(ctx.from.id));
    await отправитьПлан(coachDeps(ctx.chat.id), сообщ);
    return;
  }
  if (ЖДЁМ_РАЗБОР.has(ctx.from.id) || ЖДЁМ_РАЗБОР.has(String(ctx.from.id))) {
    ЖДЁМ_РАЗБОР.delete(ctx.from.id); ЖДЁМ_РАЗБОР.delete(String(ctx.from.id));
    await вечернийРазбор(coachDeps(ctx.chat.id), сообщ);
    return;
  }
...
async function запустить() {
  ...
  try {
    await bot.api.setMyCommands([
      ...
      { command: 'money',     description: '💰 Деньги: платежи, долги, лиды' },
  ...
  зарегистрироватьОбработчики({ bot, supa, openai, ownerTgId: OWNER_TG_ID, ждёмРазбор: ЖДЁМ_РАЗБОР });
